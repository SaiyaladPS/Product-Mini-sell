<?php
    ...
    if (!isset($_SESSION['login_admin'])) {
        ...
        if (isset($_POST['search'])) {
            $name_id_order = $_POST['select'];
// ດືງມາຈາກພື້ນຂໍ້ມູນ
        $sql_order_select = "SELECT * FROM orders INNER JOIN unit ON orders.unit_u_id = unit.u_id INNER JOIN type
         ON orders.Type_T_id = type.T_id INNER JOIN goods ON orders.goods_g_id = goods.g_id WHERE o_name LIKE '%$name_id_order%' ORDER BY o_name ASC";
         $query_order_select = mysqli_query($conn,$sql_order_select);
         $count = mysqli_num_rows($query_order_select);

         $_SESSION['success'] = "<script>
         Swal.fire({
                 icon: 'success',
                 title: 'ຄົ້ນຫາຂໍ້ມູນທີ່ຕົງກັບ $name_id_order ພົບ $count ລາຍງານ',
...
?>
      ...
      <a href="order.php" class="btn btn-outline-success mt-3">ກັບຄືນ</a>  
            <?php if ($count == 0) {?>
        ...
        <?php } else { ?>
          <h3 class="text-center mt-2">ລາຍການຂາຍທັງໝົດ</h3>
                <table class="table table-striped table-hover" style="font-size: 12px;">
                    ...
                    <tbody>
                        <?php while($select_order_row = mysqli_fetch_assoc($query_order_select)) { ?>
                                <tr>
